...
export function CardModal(props:NumberProps) {
  ...
  function resetModal(){
    ...
    setMachineBairro(props.machine.addressLabel.split(' - ').slice(1).join(' - '))
      ...
      {showModal ? (
        <>
          <div className="flex items-center justify-center min-h-screen overflow-x-hidden overflow-y-auto fixed inset-0 z-50">
            <div className={`text-center bg-gray-1000 shadow-lg shadow-gray-500 rounded-2xl w-[60vw] h-[40rem] pt-12`}>
                ...
                <form 
                  ...
                  className="flex flex-col">
                    ...
                    <div className="relative mt-2 mb-8 w-full flex items-start flex-col gap-y-2">
                      <label htmlFor="programName" className={`text-generic-fields font-bold`}>Nome</label>
                    ...
                    <div className="relative mt-2 mb-8 w-full flex items-start flex-col gap-y-2">
                      <label htmlFor="machineRua" className={`text-generic-fields font-bold`}>Rua</label>
                      <input className={`focus:ring-0 focus:border-1 focus:outline-none appearance-none leading-tight focus:border-white placeholder:text-generic-fields placeholder:font-semibold w-full border-none outline-none rounded-xl py-[1em] pl-[2em] pr-[4em] text-sm bg-generic-bgLight shadow-input hover:shadow-input-hover-focus focus:shadow-input-hover-focus`} 
                                id="machineRua" 
                                spellCheck={false}
                                onChange={(e)=>{setMachineRua(e.target.value)}}
                                value={machineRua}            
                                required/>
                    </div>

                    <div className="relative mt-2 mb-8 w-full flex items-start flex-col gap-y-2">
                      <label htmlFor="machineBairro" className={`text-generic-fields font-bold`}>Bairro</label>
                      <input className={`focus:ring-0 focus:border-1 focus:outline-none appearance-none leading-tight focus:border-white placeholder:text-generic-fields placeholder:font-semibold w-full border-none outline-none rounded-xl py-[1em] pl-[2em] pr-[4em] text-sm bg-generic-bgLight shadow-input hover:shadow-input-hover-focus focus:shadow-input-hover-focus`} 
                                id="machineBairro" 
                                spellCheck={false}
                                onChange={(e)=>{setMachineBairro(e.target.value)}}
                                value={machineBairro}            
                                required/>
                    </div>
                    ...
                    <div className="flex flex-row gap-x-5 justify-between items-center px-5">
                      <div className="mt-2 mb-8 flex items-start flex-col gap-y-2">
                        <label htmlFor="machineAtiva" className="text-generic-fields font-bold">Ativa</label>
                        <button
                          id="machineAtiva"
                          ...
                          onClick={(e: React.MouseEvent<HTMLButtonElement>)=>{e.preventDefault(); setAtiva(!ativa);}}
                          aria-label={ativa ? 'Ativo' : 'Não Ativo'}
                        >
                          <span className={`absolute left-0 inline-block w-[1.5em] h-[1.5em] transform transition-transform duration-300 ease-in-out bg-white rounded-full ${ativa ? 'translate-x-full' : 'translate-x-0'}`}></span>
                        </button>
                      </div>
                      ...
                      <div className="flex flex-row gap-x-10">
                        ...
                        <button 
                          title="Fechar"
                          type="button" 
                          className={"leading-none border-none outline-none rounded-xl p-4 px-7 cursor-pointer mb-6 shadow-button hover:shadow-button-hover-focus focus:shadow-button-hover-focus"}
